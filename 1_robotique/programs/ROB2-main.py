...
while True:
    if button_a.was_pressed(): # permet de changer de programme
        prog = (prog + 1) % 10
        display.show(prog)
        np.clear()
        music.pitch(440, 20)

    if pin_logo.is_touched(): # lorsque le logo pin est touché, il y a une musique qui se lance et ralentit les couleurs
        j = (j + 1) % len(colors) # permet de changer la couleur si on touche le logo
        music.pitch(540, 20)
        sleep(100)
